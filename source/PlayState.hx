...
class PlayState extends MusicBeatState
{
	...
	private var healthBarBf:FlxBar;
	private var healthBarBfPixel:FlxBar;
	private var healthBarObjection:FlxBar;
	private var healthBarPicoGood:FlxBar;
	private var healthBarDad:FlxBar;
	private var healthBarGf:FlxBar;
	private var healthBarSpooky:FlxBar;
	private var healthBarPicoBad:FlxBar;
	private var healthBarMom:FlxBar;
	private var healthBarAyana:FlxBar;
	private var healthBarSenpai:FlxBar;
	private var healthBarSpirit:FlxBar;
	private var healthBarCC:FlxBar;
	private var healthBarToads:FlxBar;
	private var healthBarParents:FlxBar;
	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'spooky-month' | 'spooky month':
				dialogue = CoolUtil.coolTextFile(Paths.txt('spooky-month/spookyDialogue'));
			case 'touch-tone-telephone' | 'touch tone telephone':
				dialogue = CoolUtil.coolTextFile(Paths.txt('touch tone telephone/telephoneDialogue'));
			
		}

		switch(SONG.song.toLowerCase())
		{
			case 'spooky month' | 'spookeez' | 'monster' | 'south' | 'eerie' | 'toadhemian rhapsody': 
			{
				curStage = 'spooky';
				halloweenLevel = true;
			...
			case 'pico' | 'blammed' | 'philly' | 'pico again' | 'pico-but-three' | 'pico but three' | 'llamas-with-hats' | 'llamas with hats': 
					{
					curStage = 'philly';
			...
			case 'milf' | 'satin-panties' | 'high' | 'gas gas gas' | 'spider dance':
			{
					curStage = 'limo';
					defaultCamZoom = 0.90;
			...
			case 'something spiteful':
				{
						curStage = 'limoalt';
						defaultCamZoom = 0.90;
	
						var skyBG:FlxSprite = new FlxSprite(-120, -50).loadGraphic(Paths.image('limoalt/limoSunset'));
						skyBG.scrollFactor.set(0.1, 0.1);
						add(skyBG);
	
						var bgLimo:FlxSprite = new FlxSprite(-200, 480);
						bgLimo.frames = Paths.getSparrowAtlas('limoalt/bgLimo');
						bgLimo.animation.addByPrefix('drive', "background limo pink", 24);
						bgLimo.animation.play('drive');
						bgLimo.scrollFactor.set(0.4, 0.4);
						add(bgLimo);
	
						var bgDancer1:FlxSprite = new FlxSprite(200, 60);
						bgDancer1.frames = Paths.getSparrowAtlas('limoalt/limoDancer');
						bgDancer1.animation.addByPrefix('drive', "bg dancer sketch PINK", 24);
						bgDancer1.animation.play('drive');
						bgDancer1.scrollFactor.set(0.4, 0.4);
						add(bgDancer1);

						var bgDancer2:FlxSprite = new FlxSprite(500, 60);
						bgDancer2.frames = Paths.getSparrowAtlas('limoalt/limoDancer');
						bgDancer2.animation.addByPrefix('drive', "bg dancer sketch PINK", 24);
						bgDancer2.animation.play('drive');
						bgDancer2.scrollFactor.set(0.4, 0.4);
						add(bgDancer2);

						var bgDancer3:FlxSprite = new FlxSprite(800, 60);
						bgDancer3.frames = Paths.getSparrowAtlas('limoalt/limoDancer');
						bgDancer3.animation.addByPrefix('drive', "bg dancer sketch PINK", 24);
						bgDancer3.animation.play('drive');
						bgDancer3.scrollFactor.set(0.4, 0.4);
						add(bgDancer3);

						var bgDancer4:FlxSprite = new FlxSprite(1100, 60);
						bgDancer4.frames = Paths.getSparrowAtlas('limoalt/limoDancer');
						bgDancer4.animation.addByPrefix('drive', "bg dancer sketch PINK", 24);
						bgDancer4.animation.play('drive');
						bgDancer4.scrollFactor.set(0.4, 0.4);
						add(bgDancer4);
	
						var overlayShit:FlxSprite = new FlxSprite(-500, -600).loadGraphic(Paths.image('limoalt/limoOverlay'));
						overlayShit.alpha = 0.5;
						// add(overlayShit);
						...
						var limoTex = Paths.getSparrowAtlas('limoalt/limoDrive');
						...
						fastCar = new FlxSprite(-300, 160).loadGraphic(Paths.image('limoalt/fastCarLol'));
						// add(limo);
				}
			case 'bad apple':
				{
					...
					var skyBG:FlxSprite = new FlxSprite(-120, -50).loadGraphic(Paths.image('bad apple/limoSunset'));
					skyBG.scrollFactor.set(0.1, 0.1);
					add(skyBG);
				}
			case 'four brave knights' | 'space pirates' | 'race around the world':
				{
					defaultCamZoom = 0.9;
					curStage = 'shop';
					var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('cc/stageback'));
					bg.antialiasing = true;
					bg.scrollFactor.set(0.9, 0.9);
					bg.active = false;
					add(bg);

					var stageFront:FlxSprite = new FlxSprite(-650, 600).loadGraphic(Paths.image('cc/stagefront'));
					...
					var stageCurtains:FlxSprite = new FlxSprite(-500, -300).loadGraphic(Paths.image('cc/stagecurtains'));
			...
			case 'objection':
			{
					...
					upperBoppers = new FlxSprite(-240, -90);
					upperBoppers.frames = Paths.getSparrowAtlas('objection/upperBop');
					upperBoppers.animation.addByPrefix('bop', "Upper Crowd Bob", 24, false);
					upperBoppers.antialiasing = true;
					upperBoppers.scrollFactor.set(0.33, 0.33);
					upperBoppers.setGraphicSize(Std.int(upperBoppers.width * 0.85));
					...
					var tree:FlxSprite = new FlxSprite(370, -250).loadGraphic(Paths.image('objection/christmasTree'));
					tree.antialiasing = true;
					tree.scrollFactor.set(0.40, 0.40);
					add(tree);

					bottomBoppers = new FlxSprite(-300, 140);
					bottomBoppers.frames = Paths.getSparrowAtlas('objection/bottomBop');
					bottomBoppers.animation.addByPrefix('bop', 'Bottom Level Boppers', 24, false);
					bottomBoppers.antialiasing = true;
						bottomBoppers.scrollFactor.set(0.9, 0.9);
						bottomBoppers.setGraphicSize(Std.int(bottomBoppers.width * 1));
					...
					santa = new FlxSprite(-840, 150);
					santa.frames = Paths.getSparrowAtlas('objection/santa');
					santa.animation.addByPrefix('idle', 'santa idle in fear', 24, false);
					santa.antialiasing = true;
					add(santa);
			}
			case 'winter-horrorland' | 'touch tone telephone':
			{
					curStage = 'mallEvil';
		...
		switch (curStage)
		{
			case 'limo':
				gfVersion = 'gf-car';
			case 'limoalt':
				gfVersion = 'gf-car-alt';
			case 'objection':
				gfVersion = 'edgeworth';
			case 'bad apple':
				gfVersion = 'bad apple gf';
			case 'mall' | 'mallEvil':
		...
		var bruh:DialogueBox = new DialogueBox(false, dialogue);
		 bruh.x += 70;
		// bruh.y += 70;
		bruh.scrollFactor.set();
		bruh.finishThing = startCountdown;
		...
		if (FlxG.save.data.songPosition) // I dont wanna talk about this code :(
			{
				...
				if (FlxG.save.data.downscroll)
					songName.y -= 3;
				songName.setFormat(Paths.font("FightNight.ttf"), 16, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE,FlxColor.BLACK);
		...
		healthBarBf = new FlxBar(healthBarBG.x + 4, healthBarBG.y + 4, RIGHT_TO_LEFT, Std.int(healthBarBG.width - 8), Std.int(healthBarBG.height - 8), this,
				'health', 0, 2);
			healthBarBf.scrollFactor.set();
			healthBarBf.createFilledBar(0x00FF0000, 0xFF31b0d1);
			// healthBarBf

		healthBarBfPixel = new FlxBar(healthBarBG.x + 4, healthBarBG.y + 4, RIGHT_TO_LEFT, Std.int(healthBarBG.width - 8), Std.int(healthBarBG.height - 8), this,
				'health', 0, 2);
			healthBarBfPixel.scrollFactor.set();
			healthBarBfPixel.createFilledBar(0x00FF0000, 0xFF7bd6f6);
			// healthBarBfPixel

		healthBarObjection = new FlxBar(healthBarBG.x + 4, healthBarBG.y + 4, RIGHT_TO_LEFT, Std.int(healthBarBG.width - 8), Std.int(healthBarBG.height - 8), this,
				'health', 0, 2);
			healthBarObjection.scrollFactor.set();
			healthBarObjection.createFilledBar(0x00FF0000, 0xFF4a9bf8);
			// healthBarObjection

		healthBarPicoGood = new FlxBar(healthBarBG.x + 4, healthBarBG.y + 4, RIGHT_TO_LEFT, Std.int(healthBarBG.width - 8), Std.int(healthBarBG.height - 8), this,
				'health', 0, 2);
			healthBarPicoGood.scrollFactor.set();
			healthBarPicoGood.createFilledBar(0x00FF0000, 0xFFb7d855);
			// healthBarPicoGood

		healthBarDad = new FlxBar(healthBarBG.x + 4, healthBarBG.y + 4, RIGHT_TO_LEFT, Std.int(healthBarBG.width - 8), Std.int(healthBarBG.height - 8), this,
				'health', 0, 2);
			healthBarDad.scrollFactor.set();
			healthBarDad.createFilledBar(0xFFaf66ce, 0x00FF0000);
			// healthBarDad

		healthBarGf = new FlxBar(healthBarBG.x + 4, healthBarBG.y + 4, RIGHT_TO_LEFT, Std.int(healthBarBG.width - 8), Std.int(healthBarBG.height - 8), this,
				'health', 0, 2);
			healthBarGf.scrollFactor.set();
			healthBarGf.createFilledBar(0xFFa5004d, 0x0066FF33);
			// healthBarGf

		healthBarSpooky = new FlxBar(healthBarBG.x + 4, healthBarBG.y + 4, RIGHT_TO_LEFT, Std.int(healthBarBG.width - 8), Std.int(healthBarBG.height - 8), this,
				'health', 0, 2);
			healthBarSpooky.scrollFactor.set();
			healthBarSpooky.createFilledBar(0xFFd57e00, 0x0066FF33);
			// healthBarSpooky

		healthBarPicoBad = new FlxBar(healthBarBG.x + 4, healthBarBG.y + 4, RIGHT_TO_LEFT, Std.int(healthBarBG.width - 8), Std.int(healthBarBG.height - 8), this,
				'health', 0, 2);
			healthBarPicoBad.scrollFactor.set();
			healthBarPicoBad.createFilledBar(0xFFb7d855, 0x0066FF33);
			// healthBarPicoBad

		healthBarMom = new FlxBar(healthBarBG.x + 4, healthBarBG.y + 4, RIGHT_TO_LEFT, Std.int(healthBarBG.width - 8), Std.int(healthBarBG.height - 8), this,
				'health', 0, 2);
			healthBarMom.scrollFactor.set();
			healthBarMom.createFilledBar(0xFFd8558e, 0x0066FF33);
			// healthBarMom

		healthBarAyana = new FlxBar(healthBarBG.x + 4, healthBarBG.y + 4, RIGHT_TO_LEFT, Std.int(healthBarBG.width - 8), Std.int(healthBarBG.height - 8), this,
				'health', 0, 2);
			healthBarAyana.scrollFactor.set();
			healthBarAyana.createFilledBar(0xFF65ffc4, 0x0066FF33);
			// healthBarAyana

		healthBarSenpai = new FlxBar(healthBarBG.x + 4, healthBarBG.y + 4, RIGHT_TO_LEFT, Std.int(healthBarBG.width - 8), Std.int(healthBarBG.height - 8), this,
				'health', 0, 2);
			healthBarSenpai.scrollFactor.set();
			healthBarSenpai.createFilledBar(0xFFffaa6f, 0x0066FF33);
			// healthBarSenpai

		healthBarSpirit = new FlxBar(healthBarBG.x + 4, healthBarBG.y + 4, RIGHT_TO_LEFT, Std.int(healthBarBG.width - 8), Std.int(healthBarBG.height - 8), this,
			'health', 0, 2);
			healthBarSpirit.scrollFactor.set();
			healthBarSpirit.createFilledBar(0xFFc3c3c3, 0x0066FF33);
			// healthBarSpirit
		
		healthBarCC = new FlxBar(healthBarBG.x + 4, healthBarBG.y + 4, RIGHT_TO_LEFT, Std.int(healthBarBG.width - 8), Std.int(healthBarBG.height - 8), this,
				'health', 0, 2);
			healthBarCC.scrollFactor.set();
			healthBarCC.createFilledBar(0xFFc3c3c3, 0x0066FF33);
			// healthBarCC

		healthBarParents = new FlxBar(healthBarBG.x + 4, healthBarBG.y + 4, RIGHT_TO_LEFT, Std.int(healthBarBG.width - 8), Std.int(healthBarBG.height - 8), this,
				'health', 0, 2);
			healthBarParents.scrollFactor.set();
			healthBarParents.createFilledBar(0xFFc870c3, 0x0066FF33);
			// healthBarParents

		healthBarToads = new FlxBar(healthBarBG.x + 4, healthBarBG.y + 4, RIGHT_TO_LEFT, Std.int(healthBarBG.width - 8), Std.int(healthBarBG.height - 8), this,
				'health', 0, 2);
			healthBarToads.scrollFactor.set();
			healthBarToads.createFilledBar(0xFFa524b8, 0x0066FF33);
			// healthBarToads
			...
			case 'spooky':
				add(healthBarSpooky);
			case 'pico':
				add(healthBarPicoBad);
			case 'mom' | 'mom-car' | 'bad apple mom':
				add(healthBarMom);
			case 'the ex':
				add(healthBarAyana);
			case 'senpai':
			...
			switch (SONG.player1)
			{
			case 'bf' | 'bf-car' | 'bf-christmas' | 'bad apple bf':	
				add(healthBarBf);
			case 'bf-pixel':
				add(healthBarBfPixel);
			case 'Wright':
				add(healthBarObjection);
			case 'pico':
				add(healthBarPicoGood);
			};

		// Add Kade Engine watermark
		kadeEngineWatermark = new FlxText(4,healthBarBG.y + 50,0,SONG.song + " " + (storyDifficulty == 2 ? "Hard" : storyDifficulty == 3 ? "bruh" : storyDifficulty == 1 ? "Normal" : "Easy") + (Main.watermarks ? " - KE " + MainMenuState.kadeEngineVer : ""), 16);
		...
		healthBarBf.cameras = [camHUD];
		healthBarBfPixel.cameras = [camHUD];
		healthBarObjection.cameras = [camHUD];
		healthBarPicoGood.cameras = [camHUD];
		healthBarDad.cameras = [camHUD];
		healthBarGf.cameras = [camHUD];
		healthBarSpooky.cameras = [camHUD];
		healthBarPicoBad.cameras = [camHUD];
		healthBarAyana.cameras = [camHUD];
		healthBarMom.cameras = [camHUD];
		healthBarCC.cameras = [camHUD];
		healthBarParents.cameras = [camHUD];
		healthBarSenpai.cameras = [camHUD];
		healthBarSpirit.cameras = [camHUD];
		healthBarToads.cameras = [camHUD];
		...
		if (executeModchart) // dude I hate lua (jkjkjkjk)
			{
				...
				setVar("showCutscenes", FlxG.save.data.alwaysShowCutscenes);
	...
	function startSong():Void
	{
		...
		if (FlxG.save.data.songPosition)
		{
			...
			songPosBar = new FlxBar(songPosBG.x + 4, songPosBG.y + 4, RIGHT_TO_LEFT, Std.int(songPosBG.width - 8), Std.int(songPosBG.height - 8), this,
				'songPositionBar', 0, songLength - 1000);
			songPosBar.numDivisions = 1000;
			songPosBar.scrollFactor.set();
			songPosBar.createFilledBar(0x00FF0000, 0xFFFF0000);
			add(songPosBar);
	...
	function generateRanking():String
	{
		...
		if (misses == 0 && bads == 0 && shits == 0 && goods == 0 && accuracy == 100) // just what
			ranking = "nice";
		else if (misses == 0 && bads == 0 && shits == 0 && goods == 0 && accuracy < 99.9999) // Marvelous (SICK) Full Combo
			ranking = "(MFC)";
	...
	override public function update(elapsed:Float)
	{
		...
		if (executeModchart && lua != null && songStarted)
		{
			...
			if (getVar("showOnlyStrums",'bool'))
			{
				...
				healthBarBf.visible = false;
				healthBarBfPixel.visible = false;
				healthBarObjection.visible = false;
				healthBarPicoGood.visible = false;
				healthBarDad.visible = false;
				healthBarGf.visible = false;
				healthBarSpooky.visible = false;
				healthBarPicoBad.visible = false;
				healthBarCC.visible = false;
				healthBarToads.visible = false;
				healthBarAyana.visible = false;
				healthBarSenpai.visible = false;
				healthBarSpirit.visible = false;
				healthBarMom.visible = false;
				healthBarParents.visible = false;
	...
	*/
	...
	override function stepHit()
	{
		...
		if (dad.curCharacter == 'toads' && curStep % 4 == 2)
			{
				// dad.dance();
			}
